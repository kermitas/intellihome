...
//add your function definitions for the project ArduinoUnderEclipse here
	...
	*/
	...
	static const unsigned char sendResponseSubTaskChunkedStreamBufferSize = 240;//INCOMMING_BUFFER_SIZE_FOR_COMMUNICATION_WITH_RS232_WIFI - 10;
	...
	*/
	...
	static const byte singleArraySize = 16;

	byte incommingArr7[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR7( &incommingArr7[0] , NULL );

	byte incommingArr6[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR6( &incommingArr6[0] , &incommingARBR7 );

	byte incommingArr5[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR5( &incommingArr5[0] , &incommingARBR6 );

	byte incommingArr4[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR4( &incommingArr4[0] , &incommingARBR5 );

	byte incommingArr3[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR3( &incommingArr3[0] , &incommingARBR4 );

	byte incommingArr2[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR2( &incommingArr2[0] , &incommingARBR3 );

	byte incommingArr1[ singleArraySize ];
	AnyRingBufferRecord<byte> incommingARBR1( &incommingArr1[0] , &incommingARBR2 );

	AnyRingBuffer<byte> incommingARB( &incommingARBR1 , singleArraySize );



	//byte outgoingArr10[ singleArraySize ];
	//AnyRingBufferRecord<byte> outgoingARBR10( &outgoingArr10[0] , NULL );

	//byte outgoingArr9[ singleArraySize ];
	//AnyRingBufferRecord<byte> outgoingARBR9( &outgoingArr9[0] , &outgoingARBR10 );

	byte outgoingArr8[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR8( &outgoingArr8[0] , NULL );

	byte outgoingArr7[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR7( &outgoingArr7[0] , &outgoingARBR8 );

	byte outgoingArr6[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR6( &outgoingArr6[0] , &outgoingARBR7 );

	byte outgoingArr5[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR5( &outgoingArr5[0] , &outgoingARBR6 );

	byte outgoingArr4[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR4( &outgoingArr4[0] , &outgoingARBR5 );

	byte outgoingArr3[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR3( &outgoingArr3[0] , &outgoingARBR4 );

	byte outgoingArr2[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR2( &outgoingArr2[0] , &outgoingARBR3 );

	byte outgoingArr1[ singleArraySize ];
	AnyRingBufferRecord<byte> outgoingARBR1( &outgoingArr1[0] , &outgoingARBR2 );

	AnyRingBuffer<byte> outgoingARB( &outgoingARBR1 , singleArraySize );
