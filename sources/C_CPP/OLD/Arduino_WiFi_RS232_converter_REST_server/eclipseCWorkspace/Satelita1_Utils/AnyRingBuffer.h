...
unsigned int AnyRingBuffer<DataType>::write( DataType* fromArray , unsigned int howManyWriteFromArray , bool stricSize )
{
	...
	while( howManyWriteFromArray > 0 && writePositionRecord != NULL )
	{
		//if( p == NULL ) break;
		...
		memcpy( writePositionRecord->array+writePositionInSingleArray , fromArray+alreadyWrote , toWrite );
		alreadyWrote += toWrite;
		howManyWriteFromArray -= toWrite;

		//writePosition += toWrite;

		//if( writePosition % sizeOfSingleArray == 0 )
		//{
			//p = p->next;
...
// TODO wielki problem, writePositionRecord moze byc czasem wysterowane na NULL i co teraz? jak powrocic go jesli troche wolnej przestrzenii sie pojawi?
